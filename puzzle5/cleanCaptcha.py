...
dirpath = '/Users/likeaj6/Desktop/wd/login/puzzle5/'
img_fnames = [ os.path.join(dirpath, x) for x in os.listdir( dirpath ) if x.endswith(img_ext) ]

# cv2.imshow('test', dists_display)
outpath = '/Users/likeaj6/Desktop/wd/login/puzzle5/processed/'

clean_fnames = [ os.path.splitext(os.path.basename(x))[0] +'.png' for x in img_fnames ]
clean_fnames = [ os.path.join( 'test/', x ) for x in clean_fnames ]
...
# for i in range(1):
    ...
    image = cv2.bilateralFilter(image,70,200,200)
